...
if telemetry_file:
    with tabs[tab_index]:
        ...
        col_region, col_fw, col_dt = st.columns(3)
        with col_region:
        ...
        with col_dt:
            tel_dates = tdf["telemetry_updated_at"].dropna()
            if not tel_dates.empty:
                hard_start = date(2026, 5, 1)
                data_max = tel_dates.max().date()
                # Hard floor: never allow selecting before 1 May 2026
                min_d = hard_start
                max_d = data_max if data_max >= hard_start else hard_start
                date_sel = st.date_input(
                    "Telemetry reported between",
                    value=(min_d, max_d),
        ...
        # Resolve the date range safely (date_input can return 1 or 2 values mid-selection)
        start_d = end_d = None
        if date_sel:
            if isinstance(date_sel, (list, tuple)):
                start_d = date_sel[0]
                end_d = date_sel[1] if len(date_sel) > 1 else date_sel[0]
            else:
                start_d = end_d = date_sel
            adopt = adopt[adopt["telemetry_updated_at"].notna()]
            adopt = adopt[
                (adopt["telemetry_updated_at"].dt.date >= start_d)
                & (adopt["telemetry_updated_at"].dt.date <= end_d)
            ]
